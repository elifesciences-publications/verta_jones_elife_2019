...
effectTable212 = read.table('.../AseReadCountsEffects212_bestQvalue_STAR_duprem_nosexFDR10.txt',header=T,stringsAsFactors=F)
...
postscript('.../boxplotAseReadCountsEffectsAllCrossesPercentage_STAR.ps',width=12,height=7)
...
combinedEffectsPCT$cross = factor(combinedEffectsPCT$cross,levels=c('Tyne','Forss','Shiel','Litc'))
cols = brewer.pal(6,"Set1")
cols[6] = "#999999"
g = ggplot(combinedEffectsPCT)
g = g + geom_jitter(aes(class,frequency,colour=as.factor(class)),height=0,width=0.3,size=2.5)
g = g + facet_wrap(~cross,nrow=1)
g = g + theme(axis.text.x = element_text(angle=45, hjust=1,size=24,colour='black'),axis.title.x=element_blank(),panel.background=element_rect(fill='white',colour='grey78'),panel.grid.major=element_line(colour='white'),text=element_text(size=22),legend.position="none")
g = g + ylab('Percentage')
