...
```{r}
# load
normCounts = read.table('/Users/verta/Documents/Backups/MPI/PJ035/results/xlocCountTableVarTransformed.txt')
...
# 
# # save results
# save.image("~/Documents/Manuscripts/ParallelCisTrans/scripts/revised/PCAloadingPermutation.RData")

# load results
load("~/Documents/Manuscripts/ParallelCisTrans/scripts/revised/PCAloadingPermutation.RData")
```

Correct all P-values for multiple testing.
```{r}
PCfdr = p.adjust(permutedPvalues,method="fdr")
#PCqvalue = qvalue(permutedPvalues)$qvalues
```
...
# q-value (the minimum false discovery rate at which the statistic can be called significant)
#hist(PCqvalue,breaks=100)
```
